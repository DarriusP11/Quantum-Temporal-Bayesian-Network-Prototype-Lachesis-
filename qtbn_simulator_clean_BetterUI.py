...
ss_get("perplexity_api_key", _secret_or_env("perplexity_api_key", "QTBN_PERPLEXITY_API_KEY"))
...
tab_sv, tab_red, tab_meas, tab_fid, tab_presets, tab_present, tab_fx, tab_fin, tab_insider, tab_guide, tab_advanced_q, tab_qaoa, tab_sentiment, tab_prompt, tab_vqe = st.tabs(
    [
        ...
        if PORTFOLIO_SCREENSHOT_IMPORT_ENABLED:
            with st.expander("📸 Import portfolio from screenshot (Robinhood)", expanded=False):
                ...
                api_key_input = st.text_input(
                    ...
                    type="password",
                    value=st.session_state.get("portfolio_ss_api_key", ""),
                    key="portfolio_ss_api_key",
                )
                ...
                if up is not None:
                    ...
                    if st.button("Analyze screenshot → extract position", key="portfolio_ss_analyze_btn"):
                        if not allow_external:
                            ...
                            st.warning("Paste your OpenAI API key above to run extraction.")
                        else:
    ...
    **Client-Friendly Summary**
...

    {
        ...
        use_openai_fin = st.checkbox(
            ...
            value=bool(ss_get("openai_api_key", "")),
            key="financial_lachesis_use_openai",
        )
        if use_openai_fin:
            fin_api_key = st.text_input(
                "OpenAI API Key (for Lachesis)",
                type="password",
                value=ss_get("openai_api_key", ""),
                key="financial_lachesis_api_key",
            )
            if fin_api_key != ss_get("openai_api_key", ""):
                ss_set("openai_api_key", fin_api_key)
        ...
        if st.button("Ask Lachesis", key="financial_lachesis_ask_btn"):
            if not fin_question.strip():
                ...
                if use_openai_fin and ss_get("openai_api_key", "").strip():
                    fin_messages = [
                        {
                            "role": "system",
                            "content": (
...
with tab_insider:
    ...
    if st.button("Summarize with Lachesis"):
        ...
        if not filings:
            ...
            if ss_get("lachesis_mode", "local") == "openai" and ss_get("openai_api_key", ""):
                try:
                    text, _meta = _studio_run_openai_chat(
                        messages,
                        model="gpt-4o-mini",
...
Use this tab as a live explainer: change QAOA stance, priors, QAE, or macro stress, then check here to see how Lachesis’ worldview shifts.
    ...
    openai_key = st.text_input(
        "OpenAI API Key (optional, for live LLM calls)",
        type="password",
        value=ss_get("openai_api_key", ""),
    )
...
with tab_sentiment:
    ...
    if sentiment_source == "Perplexity API (live web)":
        ...
        pplx_key = st.text_input(
            "Perplexity API Key",
            type="password",
            value=ss_get("perplexity_api_key", ""),
            key="sentiment_perplexity_api_key",
            help="Stored in session state. For persistent config use QTBN_PERPLEXITY_API_KEY or .streamlit/secrets.toml.",
        )
        if pplx_key != ss_get("perplexity_api_key", ""):
            ss_set("perplexity_api_key", pplx_key)
    ...
    if st.button("Analyze Sentiment"):
        ...
        if sentiment_source == "Perplexity API (live web)":
            res = analyze_sentiment_perplexity(
                tick_list,
                api_key=ss_get("perplexity_api_key", ""),
    ...
    if use_openai:
        api_key = st.text_input(
            ...
            type="password",
            value=ss_get("openai_api_key", ""),
            key="sentiment_lacheiss_api_key",
        )
        if api_key != ss_get("openai_api_key", ""):
            ss_set("openai_api_key", api_key)
        ss_set("lachesis_mode", "openai")
    ...
    if st.button("Explain with Lachesis"):
        ...
        if audio_bytes:
            ...
            st.caption(f"Voice playback unavailable: {audio_err}")
        if ss_get("lachesis_mode", "local") != "openai" or not ss_get("openai_api_key", ""):
            st.caption("Using local fallback (OpenAI not configured).")
